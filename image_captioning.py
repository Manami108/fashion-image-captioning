...
# Lazy load images from HDF5 in batches
def load_images_generator(hdf5_path, batch_size):
    with h5py.File(hdf5_path, 'r') as h:
        ...
        for start in range(0, total_images, batch_size):
            ...
            yield preprocess_input(images)

# Extract features in batches using the generator
def extract_features_with_generator(generator, model, batch_size):
    ...
    return np.vstack(features)  # Combine all batches into one array

# Feature extractor using InceptionV3
base_model = InceptionV3(weights='imagenet')
model_extract_features = Model(inputs=base_model.input, outputs=base_model.layers[-2].output)
